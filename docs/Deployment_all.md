...
> 如果你已经在 Linux 上完成了自定义 `OpenClaw` 接入，并准备把现有数据库、智控台配置、OpenClaw 状态迁移到 Windows，请不要直接按本页“从零部署”理解。  
> 先看这两份文档：
>
> - [运行态固化与迁移](./runtime-solidification-and-migration.md)
> - [Windows 迁移落地步骤（OpenClaw 接入版）](./windows-openclaw-migration.md)
