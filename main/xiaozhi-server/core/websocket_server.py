...
from core.body_gateway_registry import BodyGatewayRegistry
...
class WebSocketServer:
    def __init__(self, config: dict):
        ...
        self.device_connections_lock = asyncio.Lock()
        self.body_registry = BodyGatewayRegistry()
    ...
    async def register_device_connection(self, conn):
        """注册在线设备连接，供其他模块按 device_id 复用。"""
        device_id = getattr(conn, "device_id", None)
        if not device_id:
            return

        async with self.device_connections_lock:
            self.device_connections[device_id] = conn
            self.logger.bind(tag=TAG).info(f"注册在线设备连接: {device_id}")
        await self.body_registry.upsert_device_connection(
            device_id,
            getattr(conn, "headers", {}).get("client-id", ""),
            getattr(conn, "client_ip", ""),
        )

    async def unregister_device_connection(self, conn):
        """移除在线设备连接。"""
        device_id = getattr(conn, "device_id", None)
        if not device_id:
            return
        ...
        async with self.device_connections_lock:
            current_conn = self.device_connections.get(device_id)
            if current_conn is conn:
                self.device_connections.pop(device_id, None)
                self.logger.bind(tag=TAG).info(f"移除在线设备连接: {device_id}")
        await self.body_registry.mark_device_offline(device_id)

    async def get_device_connection(self, device_id: str):
        """按 device_id 获取在线连接。"""
        async with self.device_connections_lock:
    ...
    async def start(self):
        ...
        await self.body_registry.start()
