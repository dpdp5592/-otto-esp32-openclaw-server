...
class ConnectionHandler:
    ...
    async def handle_connection(self, ws):
        try:
            ...
            self.websocket = ws
            if self.server:
                await self.server.register_device_connection(self)
            ...
            try:
                ...
                if self.server:
                    await self.server.unregister_device_connection(self)
    ...
    async def _initialize_private_config_async(self):
        ...
        if private_config.get("mcp_endpoint", None) is not None:
            mcp_endpoint = private_config["mcp_endpoint"]
            if isinstance(mcp_endpoint, str):
                # Java API 当前会把 /mcp/ 改写成 /call/，但 Python 侧连的是 MCP 协议入口。
                mcp_endpoint = mcp_endpoint.replace("/call/mcp/", "/mcp/")
            self.config["mcp_endpoint"] = mcp_endpoint
