        ...
        <div class="content-area">
          <el-card class="device-card" shadow="never">
            <el-table ref="deviceTable" :data="paginatedDeviceList" class="transparent-table"
              ...
              <el-table-column :label="$t('device.openClawPairCode')" align="center" width="220">
                <template slot-scope="scope">
                  <div v-if="scope.row.pairCode" class="pair-code-cell">
                    <span class="pair-code-text">{{ scope.row.pairCode }}</span>
                    <el-button size="mini" type="text" @click="copyPairCode(scope.row.pairCode)">
                      {{ $t('device.copyPairCode') }}
                    </el-button>
                  </div>
                  <span v-else class="pair-code-empty">-</span>
                </template>
              </el-table-column>
...
export default {
  ...
  methods: {
    ...
    copyPairCode(pairCode) {
      if (!pairCode) {
        return;
      }

      const onSuccess = () => {
        this.$message.success(this.$t('device.copyPairCodeSuccess'));
      };
      const onFail = () => {
        this.$message.error(this.$t('device.copyPairCodeFailed'));
      };

      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(pairCode).then(onSuccess).catch(onFail);
        return;
      }
