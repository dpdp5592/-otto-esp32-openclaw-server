...
public class DeviceController {
    ...
    public Result<List<DeviceEntity>> getUserDevices(@PathVariable String agentId) {
        ...
        enrichOpenClawPairCodes(devices);
    ...
    private void enrichOpenClawPairCodes(List<DeviceEntity> devices) {
        if (devices == null || devices.isEmpty()) {
            return;
        }

        Map<String, Map<String, Object>> pairDeviceMap = fetchOpenClawPairDeviceMap();
        if (pairDeviceMap.isEmpty()) {
            return;
        }
        ...
        for (DeviceEntity device : devices) {
            ...
            Map<String, Object> bodyDevice = pairDeviceMap.get(device.getId());
            if (bodyDevice == null) {
                continue;
            }
            Object pairCode = bodyDevice.get("pair_code");
            Object bodyType = bodyDevice.get("body_type");
            Object online = bodyDevice.get("online");
            device.setPairCode(pairCode instanceof String ? (String) pairCode : null);
            device.setBodyType(bodyType instanceof String ? (String) bodyType : null);
            device.setBodyOnline(online instanceof Boolean ? (Boolean) online : null);
        }
    }

    private Map<String, Map<String, Object>> fetchOpenClawPairDeviceMap() {
        try {
            String bridgeUrl = getOpenClawPairDevicesUrl();
            if (StringUtils.isBlank(bridgeUrl)) {
                return Collections.emptyMap();
            }

            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_JSON);
            HttpEntity<String> requestEntity = new HttpEntity<>("{\"onlineOnly\":false}", headers);
            ...
            Object ok = responseMap.get("ok");
            if (!(ok instanceof Boolean) || !((Boolean) ok)) {
                return Collections.emptyMap();
            }

            Object devicesObj = responseMap.get("devices");
            ...
            for (Object item : deviceList) {
                ...
                Object deviceId = rawMap.get("device_id");
                if (!(deviceId instanceof String deviceIdText) || StringUtils.isBlank(deviceIdText)) {
                    continue;
                }
                ...
                result.put(deviceIdText, normalized);
            }
            return result;
        } catch (Exception e) {
            return Collections.emptyMap();
    ...
    private String getOpenClawPairDevicesUrl() {
        String wsUrlText = sysParamsService.getValue("server.websocket", true);
        if (StringUtils.isBlank(wsUrlText) || "null".equalsIgnoreCase(wsUrlText)) {
            return null;
        }

        String firstUrl = wsUrlText.split(";")[0].trim();
        if (StringUtils.isBlank(firstUrl)) {
            return null;
        }

        URI wsUri = URI.create(firstUrl);
        ...
        return URI.create(String.format(
                "%s://%s:%d/openclaw/body/v1/pair/devices",
                scheme,
                host,
                8003)).toString();
    }
